...
/*
PaginaB* remove( ArvoreB *T, PaginaB *raiz, int info, int *ant) {
    for (int i = 0; i < T->MAX_C; i++) {
...
PaginaB* remove_ArvoreB( ArvoreB *T, PaginaB *raiz, PaginaB *pai, int info) {
    int i, pos;
    ...
    if (raiz->num_chaves > pos && raiz->chaves[pos] == info) {
        ...
        remove_ArvoreB( T, raiz->filhos[pos], pai, info);
}*/
